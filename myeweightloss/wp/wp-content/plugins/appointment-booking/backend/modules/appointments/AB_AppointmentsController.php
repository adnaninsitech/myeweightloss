...
class AB_AppointmentsController extends AB_Controller
{
        ...
        if ( $total ) {
            ...
            foreach ( $rows as &$row ) {
                if ( $row['payment_total'] !== null ) {
                    $row['price'] = AB_Utils::formatPrice( $row['payment_total'] ) . ' ' . implode( ', ', array_map(
                        function ( $payment ) {
    ...
    public function executeExportToCSV()
    {
        ...
        $rows =  AB_CustomerAppointment::query( 'ca' )
            ->select( 'ca.id,
        ...
        foreach( $rows as $row ) {
            ...
            if ( $row['coupon_discount'] or $row['coupon_deduction'] ) {
                ...
                $row['price'] = $coupon->apply( $row['price'] * $row['number_of_persons'] );
            }
