...
class AB_CalendarController extends AB_Controller
{
    ...
    public function executeGetDataForAppointment()
    {
        ...
        if ( $appointment->load( $this->getParameter( 'id' ) ) ) {
            ...
            $customers = AB_CustomerAppointment::query( 'ca' )
                ->select( 'ca.id,
                    ca.customer_id,
                    ca.custom_fields,
            ...
            foreach ( $customers as $customer ) {
                $response['data']['customers'][] = array(
                    ...
                    'payment' => $customer['payment_types']
                        ? AB_Utils::formatPrice( $customer['total'] ) . ' ' . implode( ', ', array_map(
                            function ( $payment ) {
                                list( $gateway, $status ) =  explode( ' ', $payment );
                                $information   = AB_Payment::typeToString( $gateway );
                                if ( $gateway != AB_Payment::TYPE_LOCAL ) {
                                    $information .= '&nbsp;<span class="ab-pay-status-' . $status . '">' . AB_Payment::statusToString( $status ) . '</span>';
                                }
                                return $information;
                            },
                    ...
                    if ( ! empty ( $customer_appointments ) ) {
                        ...
                        if ( $payment->loadBy( array( 'customer_appointment_id' => $customer_appointments[0]->get( 'id' ) ) ) ) {
                            $description[] = sprintf(
                                '<div>%s: %s %s</div>',
                                __( 'Payment', 'bookly' ),
                                AB_Utils::formatPrice( $payment->get( 'total' ) ),
                                AB_Payment::typeToString( $payment->get( 'type' ) )
                            );
                        }
                    }
                } else {
