...
<form method="post" action="<?php echo esc_url( add_query_arg( 'type', '_general' ) ) ?>" enctype="multipart/form-data" class="ab-settings-form">
    <table class="form-horizontal">
            ...
            <td class="ab-valign-top">
                <select class="form-control" name="ab_settings_minimum_time_prior_booking" id="ab_settings_minimum_time_prior_booking">
                    ...
                    <?php foreach ( array_merge( range( 1, 12 ), range( 24, 144, 24 ), range( 168, 672, 168 ) ) as $hour ) : ?>
                        <option value="<?php echo $hour ?>" <?php selected( get_option( 'ab_settings_minimum_time_prior_booking' ), $hour ) ?>><?php echo AB_DateTimeUtils::secondsToInterval( $hour * HOUR_IN_SECONDS ) ?></option>
                    <?php endforeach ?>
                </select>
            </td>
            <td class="ab-valign-top">
                <?php AB_Utils::popover( __( 'Set how late appointments can be booked (for example, require customers to book at least 1 hour before the appointment time).', 'bookly' ) ) ?>
            </td>
        </tr>
        <tr>
            <td>
            ...
            <td class="ab-valign-top">
                <select class="form-control" name="ab_settings_minimum_time_prior_cancel" id="ab_settings_minimum_time_prior_cancel">
                    ...
                    <?php foreach ( array_merge( array( 1 ),  range( 2, 12, 2 ), range( 24, 168, 24 ) ) as $hour ) : ?>
                        <option value="<?php echo $hour ?>" <?php selected( get_option( 'ab_settings_minimum_time_prior_cancel' ), $hour ) ?>><?php echo AB_DateTimeUtils::secondsToInterval( $hour * HOUR_IN_SECONDS ) ?></option>
                    <?php endforeach ?>
                </select>
            </td>
            <td class="ab-valign-top">
                <?php AB_Utils::popover( __( 'Set how late appointments can be cancelled (for example, require customers to cancel at least 1 hour before the appointment time).', 'bookly' ) ) ?>
            </td>
        </tr>
        <tr>
            <td>
            ...
            <td class="ab-valign-top">
                <?php AB_Utils::popover( __( 'Set the URL of a page that is shown to clients after they successfully cancelled their appointment.', 'bookly' ) ) ?>
            </td>
        </tr>
        <tr id="ab_settings_cancel_denied_page_url_row">
            <td>
            ...
            <td class="ab-valign-top">
                <?php AB_Utils::popover( __( 'Set the URL of a page that is shown to clients when the cancellation of appointment is not available anymore.', 'bookly' ) ) ?>
            </td>
        </tr>
        <tr>
            <td>
            ...
            <td class="ab-valign-top">
                <?php AB_Utils::popover( __( 'Set how far in the future the clients can book appointments.', 'bookly' ) ) ?>
            </td>
        </tr>
        <tr>
            <td>
        ...
        <tr>
            ...
            <td class="ab-valign-top">
                <?php AB_Utils::popover( __( 'Set the URL of a page that the user will be forwarded to after successful booking. If disabled then the default step 5 is displayed.', 'bookly' ) ) ?>
            </td>
        </tr>
        <tr>
            <td>
