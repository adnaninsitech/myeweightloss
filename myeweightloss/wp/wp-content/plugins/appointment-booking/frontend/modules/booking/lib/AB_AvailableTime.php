...
class AB_AvailableTime
{
    ...
    public function __construct( AB_UserBookingData $userData )
    {
        ...
        $this->service   = $userData->getService();
                                ...
                                if ( $removed ) {
                                    ...
                                    if (
                                        ...
                                        if ( $show_blocked_slots ) {
                                            ...
                                            $frames[] = array(
                                                'start'    => $booking['start_time'] + $time_slot_length,
                                                'end'      => $booking['end_time'] <= $intersection['end'] ? $booking['end_time'] : $intersection['end'],
                                                'staff_id' => $staff_id,
                                                'booked'   => true
                                            );
                                        }
                                    ...
                                    // Handle fully booked slots when displaying blocked slots.
                                        ...
                                        if ( $this->isWholeDayService() ) {
                                            ...
                                            $frames[] = array(
                                                'start'    => $booking['start_time'] >= $intersection['start'] ? $booking['start_time'] : $intersection['start'],
                                                'end'      => $booking['end_time'] <= $intersection['end'] ? $booking['end_time'] : $intersection['end'],
                                                'staff_id' => $staff_id,
                                                'booked'   => true
                                            );
    ...
    private function _removeTimePeriod( array $frames, $p_start, $p_end, &$removed = false )
    {
        ...
        foreach ( $frames as $frame ) {
            ...
            foreach ( $intersections as $intersection ) {
                if ( $intersection['start'] - $frame['start'] >= $this->service->get( 'duration' ) ) {
                    $result[] = array_merge( $frame, array(
                        'end' => $intersection['start'],
                    ) );
                }
                if ( $frame['end'] - $intersection['end'] >= $this->service->get( 'duration' ) ) {
                    $result[] = array_merge( $frame, array(
    ...
    private function _prepareStaffData( DateTime $start_date )
    {
        ...
        $bookings = AB_CustomerAppointment::query( 'ca' )
            ->select( 'a.id, a.staff_id, a.service_id, a.google_event_id, a.start_date, a.end_date,
        ...
        foreach ( $bookings as $item ) {
            ...
            $item['start_time'] = AB_DateTimeUtils::timeToSeconds( $s_time );
            unset ( $item['end_date'] );
            $item['end_time']   = AB_DateTimeUtils::timeToSeconds( $e_time );
            if ( $s_date != $e_date ) {
        ...
        if  ( get_option( 'ab_settings_google_two_way_sync' ) ) {
            $query = AB_Staff::query( 's' )->whereIn( 's.id', $this->staff_ids );
    ...
    public function checkBookingTime()
    {
        ...
        $padding_left  = (int) $this->service->get( 'padding_left' );
        $padding_right = (int) $this->service->get( 'padding_right' );
        $max_date = date_create( '@' . ( current_time( 'timestamp' ) + AB_Config::getMaximumAvailableDaysForBooking() * DAY_IN_SECONDS ) )->setTime( 0, 0 );

        if ( $endDate < $max_date ) {
            $query = AB_CustomerAppointment::query( 'ca' )
                ->select( 'a.*, ss.capacity, SUM(ca.number_of_persons) AS total_number_of_persons, s.padding_left, s.padding_right' )
                ->leftJoin( 'AB_Appointment', 'a', 'a.id = ca.appointment_id' )
                ->leftJoin( 'AB_StaffService', 'ss', 'ss.staff_id = a.staff_id AND ss.service_id = a.service_id' )
                ->leftJoin( 'AB_Service', 's', 's.id = a.service_id' )
                ->where( 'a.staff_id', $this->userData->get( 'staff_id' ) )
                ->groupBy( 'a.start_date' )->groupBy( 'a.staff_id' )->groupBy( 'a.service_id' )
                ->havingRaw( '( DATE_SUB(a.start_date, INTERVAL (COALESCE(s.padding_left,0) + ' . $padding_right . ') SECOND) = %s  AND  a.service_id =  %d AND ( total_number_of_persons + %s ) > ss.capacity ) OR
                              ( DATE_SUB(a.start_date, INTERVAL (COALESCE(s.padding_left,0) + ' . $padding_right . ') SECOND) = %s  AND  a.service_id <> %d) OR
                              ( DATE_SUB(a.start_date, INTERVAL (COALESCE(s.padding_left,0) + ' . $padding_right . ') SECOND) > %s  AND  DATE_ADD(a.end_date, INTERVAL (COALESCE(s.padding_right,0) + ' . $padding_left . ') SECOND) <= %s) OR
                              ( DATE_SUB(a.start_date, INTERVAL (COALESCE(s.padding_left,0) + ' . $padding_right . ') SECOND) < %s  AND  DATE_ADD(a.end_date, INTERVAL (COALESCE(s.padding_right,0) + ' . $padding_left . ') SECOND) > %s) OR
                              ( DATE_SUB(a.start_date, INTERVAL (COALESCE(s.padding_left,0) + ' . $padding_right . ') SECOND) < %s  AND  DATE_ADD(a.end_date, INTERVAL (COALESCE(s.padding_right,0) + ' . $padding_left . ') SECOND) > %s)',
                    array(  $booked_datetime, $this->userData->get( 'service_id' ), $this->userData->get( 'number_of_persons' ),
