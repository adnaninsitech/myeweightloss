...
class AB_UserBookingData
{
     ...
     *
     * @return AB_CustomerAppointment
     */
    public function save( $pending = false )
    {
    ...
    /**
     * Get service price for given step of booking..
     *
     * @param integer $step
     * @return string|false
     */
    public function getServicePriceForStep( $step )
    {
        $staff_service = new AB_StaffService();
        $staff_service->loadBy( array(
            'staff_id'   => $this->getStaffIdForStep( $step ),
            'service_id' => $this->get( 'service_id' )
        ) );
    ...
    /**
     * Get total price (with applied coupon).
     *
     * @return float
     */
    public function getTotalPrice()
    {
        $total_price = $this->getServicePriceForStep( 4 ) * $this->get( 'number_of_persons' );
        // Apply coupon.
        $coupon = $this->getCoupon();
        if ( $coupon ) {
    ...
    /**
     * Get staff id for given step of booking.
     *
     * @param integer $step
     * @return int
     */
    public function getStaffIdForStep( $step )
    {
        if ( $step <= 2 ) {
    ...
    /**
     * Get staff name for given step of booking.
     *
     * @param integer $step
     * @return string
     */
    public function getStaffNameForStep( $step )
    {
        $staff_id = $this->getStaffIdForStep( $step );
    ...
    /**
     * Set payment ( PayPal, 2Checkout, PayU Latam, Payson ) transaction status.
     *
     * @param $status
     * @param $error
     * @param $gateway
     */
    public function setPaymentStatus( $status, $error = null, $gateway = null )
    {
