...
class AB_Staff extends AB_Entity
{
    ...
    public function getAppointmentsForFC( DateTime $start_date, DateTime $end_date )
    {
        $appointments = AB_Appointment::query( 'a' )
            ->select( 'a.id, a.start_date, a.end_date,
                ...
                (SELECT SUM(ca.number_of_persons) FROM ' . AB_CustomerAppointment::getTableName() . ' ca WHERE ca.appointment_id = a.id) AS total_number_of_persons,
                ca.custom_fields,
            ...
            if ( $appointment['max_capacity'] == 1 ) {
                ...
                if ( $appointment['payment_types'] ) {
                    $desc .= sprintf(
                        '<div>%s: %s %s</div>',
                        __( 'Payment', 'bookly' ),
                        AB_Utils::formatPrice( $appointment['payment_total'] ),
                        implode( ', ', array_map(
                            'AB_Payment::typeToString',
    ...
    {
        return AB_Utils::getTranslatedString( 'staff_' . $this->get( 'id' ), $this->get( 'full_name' ) );
    }
