...
class AB_PayPal
{
    ...
    public function send_EC_Request( $form_id )
    {
        ...
        $data =
            ...
            '&NOSHIPPING=1' .
            '&RETURNURL=' . urlencode( add_query_arg( array( 'action' => 'ab-paypal-return', 'ab_fid' => $form_id ), AB_Utils::getCurrentPageURL() ) ) .
            '&CANCELURL=' . urlencode( add_query_arg( array( 'action' => 'ab-paypal-cancel', 'ab_fid' => $form_id ), AB_Utils::getCurrentPageURL() ) );

        foreach ( $this->products as $index => $product ) {
            $data .=
                "&L_PAYMENTREQUEST_0_NAME{$index}=" . urlencode( $product->name ) .
                "&L_PAYMENTREQUEST_0_DESC{$index}=" . urlencode( $product->desc ) .
        ...
        if ( 'SUCCESS' == strtoupper( $response['ACK'] ) || 'SUCCESSWITHWARNING' == strtoupper( $response['ACK'] ) ) {
            ...
            header( 'Location: ' . wp_sanitize_redirect( add_query_arg( array( 'action' => 'ab-paypal-error', 'ab_fid' => $form_id, 'error_msg' => $response['L_LONGMESSAGE0'] ), AB_Utils::getCurrentPageURL() ) ) );
            exit;
        }
    }
