...
class AB_Updates
{
    ...
    function update_8_0()
    {
        ...
        $wpdb->query( 'ALTER TABLE ' . AB_CustomerAppointment::getTableName() . ' ADD COLUMN `locale` VARCHAR(8) NULL' );
    ...
    function update_7_8_2()
    {
        ...
        $wpdb->query( 'UPDATE ' . AB_CustomerAppointment::getTableName() .' SET custom_fields = "[]" WHERE custom_fields IS NULL or custom_fields = ""' );
    }

    function update_7_8()
    {
        global $wpdb;
        $wpdb->query( 'ALTER TABLE ' . AB_Payment::getTableName() . " ADD COLUMN `status` ENUM('completed','pending') NOT NULL DEFAULT 'completed'" );
        $wpdb->query( 'ALTER TABLE ' . AB_Payment::getTableName() . " CHANGE COLUMN `type` `type` ENUM('local','coupon','paypal','authorizeNet','stripe','2checkout','payulatam','payson') NOT NULL DEFAULT 'local'" );
...
function bookly_plugin_update_db()
{
    ...
    if ( AB_Plugin::version() > $db_version && $db_version !== false ) {
